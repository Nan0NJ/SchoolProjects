...
public class Main {
    public static void main(String[] args) {
        ...
        if(length<=0){
            System.out.println("Error: Array Wrong Input!"); // Condition --> Negative array length doesn't exist! Terminate Program Here.
            System.exit(1);
        }
        int [] Array = new int[length]; // Creating the Array and Getting User Input on the Numbers.
        ...
        DisplayArrayInformation(Array); // To display all the elements of the inserted array (as they were initially inputted). (1)
        ...
        Palindrome_BiggestPalindrome(Array); // To display counter of Palindromes & The Biggest Palindrome smaller than the biggest number in the list. (13) & (14)
         // To display the Reverse order of the array from the original user input delimited by a comma.
    ...
    public static void Repetition_Max_Occurrence (int [] arr , int n) {
        ...
        if(max_occurrence==-1)
            System.out.println("\n(6)\nThere is no number with max occurrence."); // If statement to check if there is an element with the max occurrence.
        else
    ...
    public static int LargestNumber(int [] ar){
        ...
        return newAr[newAr.length-1]; // We use the sort function to get a sorted array and from there the last element is the biggest.
    }
    public static int SecondSmallestNumber(int [] ar){
        int [] newAr = sortArrayAscendingOrder(ar);
        int second_smallest=0;
        while(newAr[second_smallest]==ar[0]) {
            second_smallest++; // loop to check if the smallest number repeats.
        }
        return newAr[second_smallest];
    }
    public static void AverageAll(int [] ar){
        double sum = Sum(ar);
        double average = sum/ar.length; // Function to get the average of the sum of all elements divided by the array length.
    ...
    public static double Standard_Deviation(int [] ar) {
        ...
        for (int i=0; i<ar.length; i++) {
            standard_deviation += Math.pow(ar[i] - mean, 2); // Function to calculate the standard deviation of our array.
        }
    ...
    public static double Median(int [] ar) {
        ...
        double median;
        if (newAr.length % 2 == 0)
            median = (newAr[newAr.length / 2] + newAr[newAr.length / 2 - 1]) / 2.0; // Function to calculate the median of our array.
        else
    ...
    public static int Sum(int [] ar) {
        int sum = 0;
        for (int i = 0; i<ar.length; i++) { // Function to calculate the sum of the elements in the array,
            sum += ar[i];
        }
        return sum;
    }
    public static void Palindrome_BiggestPalindrome(int [] ar){
        ...
        for(int i=0;i<newAr.length;i++) {
            ...
            if (newAr[i] >= 0 && rev == newAr[i]) {
                ...
                counter++; // Part of function to count the palindrome numbers
                if (newAr[i] <= newAr[newAr.length - 1] && newAr[i] >= maxPalindrome) {
                    maxPalindrome = newAr[i]; // Part of function to find the biggest palindrome number.
                    maxCounter++;
                }
            }
            rev = 0;
...
//Completion of Big Project One ~ Nenad Jakovchevski 89221061 | Thank you for reading.
